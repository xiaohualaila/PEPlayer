...
public class PERendererOnline implements GLSurfaceView.Renderer {
    ...
    //构造函数
    public PERendererOnline(PEBinFile binFile, PEDecodeBuffer decodeBuffer)throws PEError {
...
//        Log.d(TAG, "onDrawFrame:openglFrameRate-->"+Define.openglFrameRate);
        glResize();
//        Log.d(TAG, "onDrawFrame: mode:"+Define.mode);
        try {
            glDraw();//opengl绘制
        }catch (PEError e){
...
//        PEMatrix.makeUnit(mMatrix);
    ...
    void initVR(){
        ...
        GLES20.glRenderbufferStorage(GLES20.GL_RENDERBUFFER, GLES20.GL_DEPTH_ATTACHMENT, width, height);//生成renderBuffer的存储空间
...
//        if(status != GLES20.GL_FRAMEBUFFER_COMPLETE){//36053
//            boolean fboUsed = false;
//            Log.d(TAG, "initTexture: status:"+status);
//        }
    }
    void useProgram(){
...
//            GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, verCoordbuffer.capacity() / 24);
//            if (Define.pointCount<verCoordbuffer.capacity()){
//                GLES20.glDrawArrays(GL_POINTS, 0, Define.pointCount);
//                GLES20.glDrawArrays(GLES20.GL_TRIANGLES, 0, Define.pointCount*3);
//            }
...
//                GLES20.glClear(GLES20.GL_COLOR_BUFFER_BIT);
        ...
        if (Define.mode == Define.MODE_ORIGINAL){
            ...
            for (int cid = 0 ;cid<cameraNum;cid++){
                do {
                    if (textures[cid] == null){
                        Log.e(TAG, "glDraw:textures[cid]: == null ");
                    }
                    if (decodeBuffer == null){
                        Log.e(TAG, "glDraw:decodeBuffer[cid]: == null ");
                    }
                    decodeBuffer.undateIfNeed(cid, textures[cid]);//更新解码好的缓存数据
//                    PEVideoTexture videoTexture = (PEVideoTexture)decodeBuffer.textureDict.get(cid);
...
//                    }
                    ...
                    switch (cameraNum){
                        case 5:
                            squares[cid].mVertexBuffer5.position(18*cid);
                            GLES20.glVertexAttribPointer(shaderVertex, 3,GLES20.GL_FLOAT,false, 12,squares[cid].mVertexBuffer5);
                            GLES20.glVertexAttribPointer(shaderTexture,2,GLES20.GL_FLOAT,false,0,squares[cid].mTextureFlipBuffer5);
                            break;
                        case 8:
                            squares[cid].mVertexBuffer8.position(18*cid);
                            GLES20.glVertexAttribPointer(shaderVertex, 3,GLES20.GL_FLOAT,false,12,squares[cid].mVertexBuffer8);
                            GLES20.glVertexAttribPointer(shaderTexture,2,GLES20.GL_FLOAT,false,0,squares[cid].mTextureFlipBuffer8);
                            break;
                        default:
                            squares[cid].mVertexBuffer.position(18*cid);
                            GLES20.glVertexAttribPointer(shaderVertex, 3,GLES20.GL_FLOAT,false,12,squares[cid].mVertexBuffer);
                            GLES20.glVertexAttribPointer(shaderTexture,2,GLES20.GL_FLOAT,false,0,squares[cid].mTextureBuffer);
                            break;
                    }
//                    // 允许顶点位置数据数组
                    GLES20.glEnableVertexAttribArray(shaderVertex);
