...
import static com.panoeye.peplayer.map.ParseXML.createXMLByDOM;
import static com.panoeye.peplayer.map.ParseXML.parseXMLByDOM;
import static com.panoeye.peplayer.map.ParseXML.editXMLByDOM;

/**
 * Created by Administrator on 2018/3/14.
 */

public class PanoMap extends AppCompatActivity implements SensorEventListener {

    private static final String TAG = "PanoMap";
    // 定位相关
    LocationClient mLocClient;
    public PanoMap.MyLocationListenner myListener = new PanoMap.MyLocationListenner();
...
//        startService(startIntent);
    ...
    public void initOverlay() {
        ...
        File file_cfg_dir = new File(sd_path.getPath() + "//pano//param");
        File file_cfg = new File(file_cfg_dir.getPath(),"cameramap.xml");

        String IP  = null;
        String bin = null;
        int cameraType = 0;

        ArrayList<Camera>cameras = (ArrayList<Camera>)parseXMLByDOM(file_cfg);
        for (Camera camera:cameras){
            Log.d(TAG, "initOverlay: id:"+camera.getId());
            if (camera.getId() != -1){
                IP = camera.getIP();
                bin = camera.getBin();
...
//                updateMapState();
                ...
                Toast.makeText(PanoMap.this,"点击位置：" + point.latitude + ", " + point.longitude,Toast.LENGTH_LONG).show();
            }
...
//                            marker.setPosition(llNew);
                            mBaiduMap.hideInfoWindow();
                            playPano(mMarkerA.getExtraInfo().getString("IP"),mMarkerA.getExtraInfo().getString("bin"),mMarkerA.getExtraInfo().getInt("cameraType"));
                        }
                    };
                    LatLng ll = marker.getPosition();
...
//                            marker.setIcon(bd);
                            ...
                            //判断文件夹是否存在
                            file_cfg_dir = new File(sd_path.getPath() + "//pano//param");
                            //判断配置文件是否存在
                            file_cfg = new File(file_cfg_dir.getPath(),"cameramap.xml");
                            createXMLByDOM(file_cfg);
...
//                            marker.remove();
                            ...
                            //判断配置文件是否存在
                            file_cfg = new File(file_cfg_dir.getPath(),"cameramap.xml");
                            editXMLByDOM(file_cfg,"2","192.168.20.168","D00010368","5","D168");
    ...
    final private Handler handler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            switch (msg.what){
                case 0:
                    pd.dismiss();
                    Toast.makeText(PanoMap.this," 相机连接成功！",Toast.LENGTH_SHORT).show();
                    break;
                case 1:
                    pd.dismiss();
                    AlertDialog.Builder builder = new AlertDialog.Builder(PanoMap.this);  //先得到构造器
                ...
                case 2:
                    ...
                    builder = new AlertDialog.Builder(PanoMap.this);  //先得到构造器
    ...
    private void playPano(String IP, final String serialNo,int cameraCnt) {
        Log.d(TAG, "playPano: IP:"+IP);
        Log.d(TAG, "playPano: serialNo:"+serialNo);
        Define.cameraCount = cameraCnt;
                ...
                while (true){
                    ...
                    if (connectManager.flagsDict.size() == Define.cameraCount){
                        if (connectManager.connErrCnt != Define.cameraCount){
                            Intent intent = new Intent(PanoMap.this,PlayOnline.class);
                            Bundle bundle = new Bundle();
